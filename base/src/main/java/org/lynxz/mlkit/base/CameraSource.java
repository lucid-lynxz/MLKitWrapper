...
public class CameraSource {
    ...
    public synchronized void stop() {
        ...
        if (processingThread != null) {
            try {
                ...
                LogWrapper.d(TAG, "Frame processing thread interrupted on release.");
            }
        ...
        if (camera != null) {
            ...
            try {
                ...
                LogWrapper.e(TAG, "Failed to clear camera preview: " + e);
            }
            camera.release();
            camera = null;
        }
    ...
    public boolean setZoom(float zoomRatio) {
        LogWrapper.d(TAG, "setZoom: " + zoomRatio);
        if (camera == null) {
            return false;
        }
    ...
    private Camera createCamera() throws IOException {
        ...
        LogWrapper.v(TAG, "Camera preview size: " + previewSize);
        ...
        if (pictureSize != null) {
            LogWrapper.v(TAG, "Camera picture size: " + pictureSize);
        ...
        if (REQUESTED_AUTO_FOCUS) {
            if (parameters
                ...
                LogWrapper.i(TAG, "Camera auto focus is not supported on this device.");
            }
        }
    ...
    public static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        if (validPreviewSizes.size() == 0) {
            LogWrapper.w(TAG, "No preview sizes have a corresponding same-aspect-ratio picture size");
            for (Camera.Size previewSize : supportedPreviewSizes) {
    ...
    private void setRotation(Camera camera, Parameters parameters, int cameraId) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                LogWrapper.e(TAG, "Bad rotation value: " + rotation);
        }
        ...
        LogWrapper.d(TAG, "Display rotation is: " + rotation);
        LogWrapper.d(TAG, "Camera face is: " + cameraInfo.facing);
        LogWrapper.d(TAG, "Camera rotation is: " + cameraInfo.orientation);
        // This value should be one of the degrees that ImageMetadata accepts: 0, 90, 180 or 270.
        LogWrapper.d(TAG, "RotationDegrees is: " + this.rotationDegrees);
        ...
        public void run() {
            ...
            while (true) {
                synchronized (lock) {
                    while (active && (pendingFrameData == null)) {
                        try {
                            ...
                            LogWrapper.d(TAG, "Frame processing loop terminated.", e);
                            return;
                        }
                    }
                ...
                try {
                    ...
                    LogWrapper.e(TAG, "Exception thrown from receiver.", t);
                } finally {
